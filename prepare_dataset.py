...
def main(annotations_path: Path, save_path: Path, no_split: bool, reduce: float):
    if isinstance(annotations_path, bytes):
        annotations_path = Path(annotations_path.decode('utf-8'))
    if isinstance(save_path, bytes):
        save_path = Path(save_path.decode('utf-8'))
    annotations: list[Path]
    outputs: list[Path]
    if no_split:
        annotations = [annotations_path / 'annon.json']
    ...
    for annotation_path, output_path in tqdm(zip(annotations, outputs)):
        ...
        for index_id, ann_ids in tqdm(annotations, leave=False):
            ...
            for ann_id in ann_ids:
                ...
                cv2.imwrite(str(output_path / f'{ann_id}.png'), img)
        json.dump(words, (output_path / 'words.json').open('w', encoding='utf-8'), ensure_ascii=False)
